...
- **제출(submit) 재시도**: 최대 `mock-worker.submit.max-attempts`(기본 5회)까지 재시도합니다.
- **폴링(poll) 재시도**: 횟수 제한 없이 재시도하되, `mock-worker.poll.deadline`(기본 10분) 초과 시 FAILED 처리합니다.
- **폴링 데드라인 기준**: `lastProgressAt`(마지막으로 Mock Worker가 정상 응답한 시각)을 기준으로 계산합니다. 일시적 fetch 오류가 발생해도 이 값은 갱신되지 않으므로, Mock Worker 장애가 계속되어도 데드라인이 밀리지 않습니다.
- **지수 백오프**: 초기 대기 시간 × 2^n, 상한값(ceiling)으로 제한합니다.
- **서킷 브레이커(Resilience4j)**: 최근 10번 호출 중 실패율 50% 초과 시 서킷을 OPEN으로 전이하고 30초간 호출을 차단합니다. OPEN 상태에서는 `isTransient=true` 예외를 즉시 반환해 재시도 백오프 흐름과 연동합니다. 4xx 등 영구 오류(`isTransient=false`)는 서킷 실패율 집계에서 제외해, 요청 오류가 외부 서비스 장애 신호로 오판되지 않도록 했습니다.
...
- 키 미전달 → 페이로드 해시로 자동 생성 (`auto-{sha256(imageUrl)}`). 이 경우 같은 URL을 두 번 요청하면 동일 작업이 반환됩니다. 다른 작업으로 처리하려면 클라이언트가 명시적 키를 전달해야 합니다.
